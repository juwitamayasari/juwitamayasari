...
$tampil="select * from karyawan where $skripsi like '%$search%' order by id_karyawan limit $posisi,$batas";
...
echo"<td> <a href='update.php?id=$row[id_karyawan]'> Edit |
		<a href='delete.php?id=$row[id_karyawan]' onclick='return confirm(\"Anda Yakin?\")'> Hapus </a></td>";
$no++;
}
echo "</table>";
?>
...
if ( ! $_GET['input'] )
	  ...
	  { $skripsi = $_GET['kombo'];}
$tampil2="select * from karyawan where $skripsi like '%$search%' order by id_karyawan limit $posisi,$batas";
